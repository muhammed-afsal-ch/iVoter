...
function VicePresidential() {
  const [cookies, setCookies] = useCookies(["user"]);
  const [user, setUser] = useState({});
  const navigate = useNavigate();
  const position = "Vice-Presidential";
  const [VicePresidentialCandidates, setVicePresidentialCandidates] = useState(
    []
  );
    ...
    if (cookies.user === undefined) {
      ...
      axios
        ...
        .then((res) => {
          ...
          if (res.data.votestatus[0].vice_presidential === "") {
            console.log("Is empty vice presidential");
  ...
  useEffect(() => {
    axios
      .post("/api/2175/vote/getAllVicePresidential", { position })
      .then((res) => {
        console.log(res.data.VicePresidentialCandidates);
        setVicePresidentialCandidates(res.data.VicePresidentialCandidates);
      })
      .catch((err) => console.error(err));
  ...
  function handleVote(event) {
    ...
    if (clicked === true) {
      axios
        .post("/api/2175/vote/addVicePresidential", vote)
        .then((res) => {
          console.log(res.data);
          if (res.data.ADD_VicePresidential_STATUS === "ADDED") {
            navigate("/vote/Treasurer");
          } else {
          }
        })
    ...
    <div>
      ...
      >
        <table style={{ background: "white" }}>
          <h1 style={{ textAlign: "center" }}>Vice Presidential</h1>
          <tr>
            <th>
              <h2>Candidates</h2>
            </th>
          ...
          {VicePresidentialCandidates.map((item, key) => {
            console.log(item);
            return (
              <tr key={key}>
                <div>
...
export default VicePresidential;
