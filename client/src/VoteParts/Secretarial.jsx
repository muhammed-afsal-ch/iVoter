...
function Secretarial() {
  ...
  const position = "Secretarial";
  const [SecretarialCandidates, setSecretarialCandidates] = useState([]);
    ...
    if (cookies.user === undefined) {
      ...
      axios
        ...
        .then((res) => {
          ...
          if (res.data.votestatus[0].secretarial === "") {
            console.log("Is empty secretarial");
  ...
  useEffect(() => {
    axios
      .post("/api/2175/vote/getAllSecretarial", { position })
      .then((res) => {
        console.log(res.data.SecretarialCandidates);
        setSecretarialCandidates(res.data.SecretarialCandidates);
      })
      .catch((err) => console.error(err));
  ...
  function handleVote(event) {
    ...
    if (clicked === true) {
      axios
        .post("/api/2175/vote/addsecretarial", vote)
        .then((res) => {
          console.log(res.data);
          if (res.data.ADD_SECRETARIAL_STATUS === "ADDED") {
            navigate("/vote/VicePresidential");
          } else {
          }
        })
    ...
    <div>
      ...
      >
        <table style={{ background: "white" }}>
          <h1 style={{ textAlign: "center" }}>Secretarial</h1>
          <tr>
            <th>
              <h2>Candidates</h2>
            </th>
          ...
          {SecretarialCandidates.map((item, key) => {
            console.log(item);
            return (
              <tr key={key}>
                <div>
