...
function Presidential() {
    ...
    if (cookies.user === undefined) {
      ...
      axios
        ...
        .then((res) => {
          ...
          if (res.data.votestatus[0].presidential === "") {
  ...
  useEffect(() => {
    axios
      .post("/api/2175/vote/getAllPresidential", { position })
      .then((res) => {
        console.log(res.data.PresidentialCandidates);
        setPresidentialCandidates(res.data.PresidentialCandidates);
      })
      .catch((err) => console.error(err));
  ...
  function handleVote(event) {
    ...
    if (clicked === true) {
      axios
        .post("/api/2175/vote/addpresidential", vote)
        .then((res) => {
          if (res.data.ADD_PRESIDENTIAL_STATUS === "ADDED") {
            navigate("/vote/Secretarial");
          } else {
          }
        })
    ...
    <div>
      ...
      >
        <table style={{ background: "white" }}>
          <h1 style={{ textAlign: "center" }}>Presidential</h1>
          <tr>
            <th>
              <h2>Candidates</h2>
            </th>
          ...
          {PresidentialCandidates.map((item, key) => {
            console.log(item);
            return (
              <tr key={key}>
                <div>
